...
Shader "VolumetricShaders/CloudLayerBackup" {
	Properties{
...
#define minHeight 200.0
#define	maxHeight 3600.0
...
#define MaxStepDist 500
	...
	{
		float StepMod = 500.0 / (float)Steps;
		...
		float3 additional = direction * (((maxHeight - minHeight) / ((float)Steps)));
		float3 additionalLight = float3(1, 1, 1) * (((maxHeight - minHeight) / ((float)Steps / (float)LightStepDiv))) * 2.0;
		...
		for (int i = 0; i < Steps; i++)
		{
			...
			float h = getH(p, original, Steps, i);
			//original *= thickness;
			//if(i > 10)
			//	return float4(original, original, original, 1);

			shadow.y = max(-h, 0.0);
			shadow.x += (max(-h, 0.0) / ((float)Steps));
			...
			lightPos += p * (((newThickestY || (!emptySpace))
				&& (when_eq(float4(lightPos.x, lightPos.y, lightPos.z, 1),
					float4(0, 0, 0, 1))))) *  shadowy;
			...
			shadowSum.y += ((shadow.y) /* (1 - shadowSum.y)*/ / Steps) * (_Density / 4)  * StepMod * shadowy;
			...
			d += length(newDist);
			//p += newDist;
			p += (float3)direction * 100.0;
			work += 1.0 * shadowy;
			//return shadowSum.y;
			lightPos = p;
			for (int u = 0; u < (float)Steps / (float)LightStepDiv; u++)
			{
				...
				//	break;
				float original2 = 0;
				float t = getH(float3(lightPos.x, lightPos.y, lightPos.z),
					original2, (float)Steps / (float)LightStepDiv, u);

				test.y = max(-t, 0.0);
				//test.x += (max(-t, 0.0) / ((float)Steps ) * Density;// (float)LightStepDiv)) * Density;
				//shadowSum2.y += test.y * (1.0 - shadowSum2.y) * shadowy2;
				//Too aggressive? (works tho)
				//shadowSum2.x += shadow.x * (shadowSum2.y) * shadowy2;
				shadowSum2.x += (((max(-t, 0.0) / ((float)Steps * (float)LightStepDiv))) * _Density * shadowy2) * brightness;
				lightPos -= ((additionalLight * _SunDir) / /*((float)Steps * (float)LightStepDiv)) * u * 100*/5);
			}
			//shadowSum.x += (shadowSum2.x * (shadowSum.y - (shadow.y * (1 - shadowSum.y)))) / ((float)Steps / (float)LightStepDiv);
			//Alpha premultiply
			...
			((float)Steps * (float)LightStepDiv / 100));*/
			/*
			shadowSum.x += (saturate((shadowSum.x * (1 - clamp(shadowSum.y, 0, 1))) +
		...
		//shadowSum.x = 0;
		//shadowSum.x = (float)Steps / (float)LightStepDiv;
		//shadowSum.x = 0.01;
		//shadowSum.x = (float)Steps;
		/*
		...
		//if (shadowSum2.x >= 1)
		//	break;
		float t = getH(float3(lightPos.x, lightPos.y, lightPos.z), (float)Steps / (float)LightStepDiv, u);
		test.y = max(-t, 0.0);
		//test.x += (max(-t, 0.0) / ((float)Steps ) * Density;// (float)LightStepDiv)) * Density;
		//shadowSum2.y += test.y * (1.0 - shadowSum2.y) * shadowy2;
		//Too aggressive? (works tho)
		//shadowSum2.x += shadow.x * (shadowSum2.y) * shadowy2;
		shadowSum2.x += (max(-t, 0.0) / ((float)Steps/ (float)LightStepDiv)) * _Density;
	...
	fragOut frag(v2f i) {
		...
		if (unity_CameraProjection[0][2] < 0) {
			i.cDir = bilerpC(_LeftEyeVectorMatrix[0],
			...
			i.cDir = bilerpC(_RightEyeVectorMatrix[0],
